...
import { Plus, Edit2, Trash2, Search, Shield, Eye, Grid } from 'lucide-react';
import { supabase, fetchBases, fetchOperadoras, syncUserOperadoras, createOperadora } from '../services/supabase';
...
const Users: React.FC<UsersProps> = ({ userRole = 'admin' }) => {
  ...
  const userGroups: {
    ...
    label: string;
    description: string;
    filter: (usuario: Usuario) => boolean;
  }[] = [
    {
      id: 'todos',
      label: 'Todos',
  ...
  const fetchModulos = async () => {
    try {
      const { data, error } = await supabase.from('modulos').select('*').eq('activo', true).order('nombre');
    ...
    try {
      ...
      const usuariosWithModules = await Promise.all(
        (data || []).map(async (usuario: any) => {
          const { data: userModules } = await supabase.from('usuarios_modulos').select('modulo_id').eq('usuario_id', usuario.id);
          return { ...usuario, modules: userModules?.map((um) => um.modulo_id) || [] };
        }),
      );
      ...
      if (ids.length > 0) {
        ...
        if (userOperadorasError) {
          console.warn('No se pudieron cargar las operadoras:', userOperadorasError);
        } else {
  ...
  const syncUserModules = async (usuarioId: string, moduleIds: string[]) => {
    await supabase.from('usuarios_modulos').delete().eq('usuario_id', usuarioId);

    if (moduleIds.length > 0) {
      const { error } = await supabase.from('usuarios_modulos').insert(
  ...
  const upsertProfile = async (userId: string, emailOverride?: string) => {
    const payload = {
      id: userId,
      email: (emailOverride ?? formData.email).trim(),
      role: formData.rol,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        // ✅ EDITAR: primero sync de Auth (email/password) vía Edge update-user
        const oldEmail = normalizeEmail(editingUser.email);
        const newEmail = normalizeEmail(formData.email);

        const wantsEmailChange = !!newEmail && newEmail !== oldEmail;
        const wantsPasswordChange = !!formData.password;

        if (wantsEmailChange || wantsPasswordChange) {
          const { data, error } = await supabase.functions.invoke('update-user', {
            body: {
              id: editingUser.id,
              email: wantsEmailChange ? newEmail : undefined,
              password: wantsPasswordChange ? formData.password : undefined,
            },
          });
          if (error) throw error;
          if ((data as any)?.error) throw new Error((data as any).error);
        ...
        // Después: actualizar usuario en tabla (campos de negocio)
        const updateData: any = {
          email: newEmail, // mantenemos consistencia
        ...
        // si seguís guardando password en tabla, update-user ya la guardó si lo programaste así.
        // Igual dejamos esto por compat: (no pisa si está vacío)
        if (formData.password) {
          updateData.password = formData.password;
        }

        const { error: updateError } = await supabase.from('usuarios').update(updateData).eq('id', editingUser.id);
        if (updateError) throw updateError;

        await syncUserModules(editingUser.id, formData.modules);
        await syncUserOperadoras(editingUser.id, formData.operadoras);

        await upsertProfile(editingUser.id, newEmail);

        alert('Usuario actualizado exitosamente');
      } else {
        // ✅ CREAR (ya lo tenías con create-user)
        ...
        // base
        await supabase.from('usuarios').update({ base_id: formData.base_id || null }).eq('id', parsedAuthUser.id);
  ...
  const filteredUsuarios = groupFilteredUsuarios.filter(
    (u) => u.nombre.toLowerCase().includes(searchTerm.toLowerCase()) || u.email.toLowerCase().includes(searchTerm.toLowerCase()),
  );
          ...
          return (
            <button
              ...
              className={`text-left rounded-2xl border p-4 transition ${
                isActive ? 'border-amber-500 bg-amber-50 shadow-sm' : 'border-slate-100 bg-white hover:border-slate-200'
              }`}
            >
              ...
              {filteredUsuarios.length === 0 ? (
                ...
                filteredUsuarios.map((usuario) => (
                  <tr key={usuario.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{getRoleBadge(usuario.rol)}</td>

                    <td className="px-6 py-4 relative">
                      <button
                        type="button"
                        onClick={() => setOpenModulesUser((current) => (current === usuario.id ? null : usuario.id))}
                      ...
                      {openModulesUser === usuario.id && (
                        <div className="absolute z-20 mt-2 w-56 rounded-2xl border border-stone-200 bg-white p-3 shadow-lg">
                          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Módulos asignados</p>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <p className="text-slate-600">{bases.find((base) => base.id === usuario.base_id)?.nombre || 'Sin base'}</p>
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {usuario.operadoras && usuario.operadoras.length > 0 ? (
                          usuario.operadoras.map((operadoraId) => {
                            const op = operadoras.find((x) => x.id === operadoraId);
                            return op ? (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <p className="text-slate-600">{new Date(usuario.created_at).toLocaleDateString('es-AR')}</p>
                    </td>
                    ...
                    {(canEdit || canDelete) && (
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex items-center gap-2">
                          {canEdit && (
                            <button onClick={() => handleEdit(usuario)} className="text-blue-600 hover:text-blue-900 p-1">
                              <Edit2 size={16} />
                            </button>
                          )}
                          {canDelete && (
                            <button onClick={() => handleDelete(usuario.id)} className="text-red-600 hover:text-red-900 p-1">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-slate-900 mb-4">{editingUser ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="border border-slate-200 rounded-2xl p-4 space-y-3">
                    ...
                    <label key={modulo.id} className="inline-flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={() => {
                          ...
                          const modules = has ? formData.modules.filter((x) => x !== modulo.id) : [...formData.modules, modulo.id];
                ...
                {modulos.length === 0 && <p className="text-sm text-slate-500">No hay modulos disponibles. Crea modulos primero.</p>}
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 text-sm text-slate-600">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 border border-slate-200 rounded-xl">
                    {operadoras.length === 0 && <div className="text-xs text-slate-400">No hay operadoras cargadas. Agrega una nueva abajo.</div>}
                      ...
                      return (
                        <label key={operadora.id} className="inline-flex items-center gap-2 text-sm">
                          <input
                            ...
                            checked={has}
                            onChange={() => {
                              const next = has ? formData.operadoras.filter((x) => x !== operadora.id) : [...formData.operadoras, operadora.id];
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 px-4 py-2 bg-slate-900 text-white rounded-lg font-medium hover:bg-slate-800 transition-colors">
